...
public:
    ...
    ros::Time sync(double c_cam, double c_ros_big, unsigned int frameID);
    double timeOffset_;

private:
    double p_cam_, p_ros_, p_out_;
...
ros::Time TimestampSynchronizer::sync(double c_cam, double c_ros_big, unsigned int frameID){
    ...
    double c_err = c_out - c_ros; // zasto ne c_out - c_ros?!
    ROS_INFO("delta_cam: %.10lf delta_ros: %.10lf delta_out: %.10lf frame_count_: %d", time_cam - p_cam_, c_ros - p_ros_, c_out - p_out_, frame_count);
